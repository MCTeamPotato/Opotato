...
public class ModsByLocation {
    ...
    public ModsByLocation(Map<Path, Set<CommonModMetadata>> locationToMod) {
        ...
        locationToMod.forEach((path, mod) -> mods.put(normalizePathString(path.toUri().toString()), mod));
    ...
    private static String normalizePathString(String path) {
        ...
        String noUnion = removePrefix(path);
    ...
    private static String removePrefix(String str) {
        return str.startsWith("union:/") ? str.substring("union:/".length()) : str;
    }

    private static String removeAndBefore(String str) {
        int toRemovePos = str.lastIndexOf("//");
        if (toRemovePos == -1) return str;
        else return str.substring(toRemovePos + "//".length());
    }
