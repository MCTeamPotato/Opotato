...
public class CommonEvents {
    ...
    public static void onHeadshot(LivingHurtEvent event) {
        ...
        if (attacked instanceof Player) {
            ...
            if (archer instanceof Player) ((Player) archer).displayClientMessage(new TranslatableComponent("headshot.opotato.on_entity"), true);
        }
        ...
        if (PotatoCommonConfig.DO_BLIND.get())
            attacked.addEffect(new MobEffectInstance(MobEffects.BLINDNESS, PotatoCommonConfig.BLIND_TICKS.get(), 3));
        if (PotatoCommonConfig.DO_NAUSEA.get())
            attacked.addEffect(new MobEffectInstance(MobEffects.CONFUSION, PotatoCommonConfig.NAUSEA_TICKS.get(), 2));
    }
